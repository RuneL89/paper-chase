# Sprint 7 — Validation, Quality & Cross-Wiki: Critic, Completeness, Lint, and Multi-Wiki Index

| Attribute | Value |
|---|---|
| Sprint ID | `sprint-07-validation-quality` |
...
- **Sprints 1–6** must be approved by the user.
- The LLM sub-agent pipeline, dynamic structure, and human approval flow are in place.
...
- `tests/writers/index.test.ts` — new tests for cross-wiki index.
...
2. The LLM Critic returns `approved` or `blocked` with structured feedback.
...
7. `index-of-indexes.md` is updated after every `ingest` run in a multi-wiki workspace.
...
6. The Critic blocks and reprocesses a chunk when it detects missing citations or un-preserved tables.
...
Preserve all context from Sprints 1–6. Every previous component must remain functional. Do not start fresh.
...
After approval, proceed to **Sprint 8 — README Documentation**: `plan/Plan_implementation/sprint-08-readme/instruction.md`.
