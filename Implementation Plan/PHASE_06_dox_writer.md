...
**Version:** 1.1.0
**Status:** Reopened (2026-07-20 amendment: workspace-level `index-of-indexes.md`)
**Date:** 2026-07-16
...
At the end of every ingest, the DOX Writer also runs a **workspace pass**: one LLM call that regenerates `wikis/index-of-indexes.md`, the workspace-level contract listing every wiki. The workspace index is **the topmost parent in the same bottom-up chain** — folder indexes (deepest-first) → wiki root index → workspace index — and follows the same rule as every other parent: it synthesizes **only** the freshly-written root `index.md` contracts of its child wikis, never the content pages inside them.
...
**Algorithm (workspace pass, amendment 2026-07-20):**

The workspace index is written **like any other parent index, using the same flow**: the ingest run first writes the folder indexes deepest-first, then the wiki root index, and only then the workspace index — so every parent in the chain reads the freshly-written contracts of its direct children. The workspace pass's only content input is the set of wiki root `index.md` files (summary-of-summaries); it must NOT read the content pages inside the child wikis.

1. List every directory in `wikis/` that contains a root `index.md`.
2. For each wiki, read its root `index.md` (freshly written this run for the ingested wiki) and compute corpus statistics (wiki count; total sources, document, entity, and topic pages across all wikis).
3. Call the LLM once with a structured prompt asking it to write a rich `wikis/index-of-indexes.md` that synthesizes the wiki root contracts (bottom-up summary-of-summaries, same pattern as folder parents).
4. Deterministic code supplies and re-imposes the exact children list (`<slug>/index.md` per wiki, vault-relative inside `wikis/`) and statistics; the LLM's frontmatter is discarded. Any LLM failure falls back to the deterministic contract.
5. The workspace index sits inside `wikis/` so the whole `wikis/` folder can be opened as one Obsidian vault where its `[[<slug>/index|<Wiki Title>]]` links resolve natively. It is outside every single-wiki vault, so `validateWiki()` does not check it.
6. Prose is written in the output language of the wiki whose ingest triggered the pass (an `outputLanguage` option; defaults to English until Phase 7 lands per-wiki language config).
...
**File:** `prompts/dox-writer-workspace.prompt.txt` (amendment 2026-07-20)

The workspace-pass variant, loaded for the single `index-of-indexes.md` call. Same rules (verbatim children/statistics/link targets, map-not-territory, no invented facts) with these differences: no `wiki` frontmatter field, no `AGENTS.md`/rolling-memory inputs (the workspace has no single constitution), the catalog section is `## Wikis`, and a language directive names the triggering ingest's output language.
