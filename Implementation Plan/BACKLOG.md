...
*Last refreshed: 2026-08-27 (B23 + B24 implemented as Phases 25 and 26 — the generic-label disambiguation and per-PDF patch-amendment canon pass is code, gates green).*
...
### ~~B23. Generic-label disambiguation via composite pages (Option E, Variant B)~~ — SCHEDULED as [PHASE_25](PHASE_25_generic_label_disambiguation.md) (canonized 2026-08-26; vision `02` §4.6 / `05` §6-§7 / `04` §3.2 Step 6b amended the same day)

- **User directive (2026-08-26):** solve the collision where generic labels such as `Indikator 2` mean different things in different source databases (DPD = first treatment contact within 10 days; HOFTER = surgery within 24 hours). The chosen direction is a composite-style disambiguation page at the generic slug, with per-source meanings as member groups.
- **Design:**
  1. **Detection in the Materializer.** After aggregation, flag slugs that match generic patterns (`isIndicatorSlug` in `src/agents/pre-merge.ts:613` and similar `table-N`/`section-N` families) AND receive evidence from multiple sources whose definitions have low substantive overlap. The Materializer already holds per-source claims/significance (`src/materializer.ts:850`, `:948`).
  2. **LLM heterogeneity judgment.** A lightweight agent reviews the generic slug with evidence grouped by source file. It returns either `split: false` (same meaning across sources) or `split: true` with distinct member titles/slugs/sources. This mirrors the existing Phase 21/22 pattern: deterministic code proposes, LLM confirms/denies.
  3. **Composite page output.** If split is confirmed, the generic slug becomes a `type: composite` page (`src/pages/composite-page.ts:60`) with one member per distinct meaning. The existing `writeCompositePage` groups evidence per member under `### <Member Title>` subheadings, so DPD and HOFTER evidence stay separate inside one page. Member pages are NOT written; the composite is the canonical page, preserving the existing composite contract (`src/materializer.ts:1369`).
  4. **Sticky decision record.** Add `disambiguate` to `CurationDecisionAction` (`src/state/curation-decisions.ts:46`) or reuse the `cluster` action with a new class (e.g., class 6 = generic-label disambiguation). Pre-apply the decision on later runs so the split is paid for once and never re-litigated.
- **Interaction with Option B Patch:** the composite page gives Option B Patch a stable, member-scoped target. Amending a generic label after a new PDF arrives means either (a) adding evidence to an existing member section (bounded patch) or (b) adding a new member when the new PDF introduces a previously unseen meaning (structural patch). Both operations are smaller than rewriting the entire conflated page.
- **Open decisions before scheduling:**
  - Should the disambiguation agent be a separate LLM call or an extension of the existing curation agent?
  - Which slug patterns beyond `indikator-N` qualify as generic? (`table-N`, `section-N`, `appendix-N`, etc.)
  - Should the page be a true `type: composite` or a new `type: disambiguation` with the same per-member rendering?
  - How do wikilinks that previously targeted the generic slug resolve once it becomes a composite?
- **Resolved in the Phase 25 canon pass (2026-08-26):** separate lightweight call (`callType: 'disambiguate'`, routed via the `curation` slot — no new Settings row); the pattern set is `indikator|indicator|table|tabel|section|afsnit|appendix|appendiks|figure|figur-N(-name)?`; the page IS a `type: composite` (class 6 — reuses the Phase 22 rendering); the composite lives AT the generic slug so pre-existing wikilinks keep resolving, and member titles join the aliases union.

### ~~B24. Per-PDF sequential ingestion with patch amendment (Option B, Patch Output)~~ — IMPLEMENTED as [PHASE_26](PHASE_26_per_pdf_patch_amendment.md) (canonized 2026-08-26; vision `04` §1/§3.2/§4/§6 + `07` §3 + `01` §4.1 amended the same day; all 11 gates green 2026-08-27, live UAT 26.1: 5 pages patched incl. a contradiction flag, $0.2288 on glm-5.3-flash)

- **User directive (2026-08-26):** each PDF runs its own mini-pipeline (extract → materialize → synthesize/amend) before the next begins, with DOX/cross-wiki/Updater deferred to after the loop — pages build incrementally instead of all at once. Existing entity/topic/comparison pages are AMENDED with only the new information (including contradiction flags with both sides quoted); the LLM performs the semantic diff (no deterministic text-diff — near-synonymous restatements must be recognized as duplicates). AskUserQuestion decisions 2026-08-26: per-PDF is the NEW DEFAULT (batch pipeline removed, not flagged); patch output ONLY from day one (no full-page-amendment intermediate stage); curation runs per PDF inside the loop (identity settled before each PDF's synthesis); Phase 26 scheduled AFTER Phase 25 so patches target stable member-scoped sections.
- **Safety design (ratified in the canon amendments):** a closed patch-operation vocabulary with NO delete/rewrite ops; deterministic applier; preservation check over the MERGED page against old ∪ new evidence; reask ≤3 then normal full synthesis on exhaustion — a page is never left half-patched; `patch-amended` is a skip-eligible synthesis mode; first-PDF behavior byte-identical to today; `.state/amendment-log.jsonl` audits every attempt with its cost evidence.
- **Evidence:** the 43-report 2025 RKKP corpus analysis — batch re-emission of dense pages on every new PDF scales output cost with accumulated page size; patch amendment scales it with the new evidence per PDF (measured example: ~1-1.5k amendment tokens vs ~4-7k full re-emission for a hospital entity page).
