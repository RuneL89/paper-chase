...
Rejection loops and criteria: LLM calls are retried only on bounded criteria — **transient** transport failures (429/5xx/network) up to 3 total attempts, **quality** failures (a synthesis page that fails the preservation check, or an unparseable/incomplete DOX Writer response) up to 3 total attempts before the deterministic fallback, and **deterministic** failures (invalid Extractor JSON, schema-validation errors, HTTP 4xx such as auth or invalid request) never retried. After the pipeline, a deterministic validation pass checks links, citations, and schema across the whole wiki and writes `.state/validation-report.json`.
...
* `metrics.json` — per-run metrics: chunks processed/skipped/failed, entities new/updated, relationships, claims (by type), pages written (by type), folders created, broken links, orphaned pages, conflicts (manual-edit vs preservation), total tokens, total cost, wall-clock time. A crash-safe preliminary metrics file is written before validation/DOX and a final one at the end of the run; a one-line summary is printed at the end of ingest.
