...
import { SlugRegistry } from '../utils/slug.js';
...
export function writeEntityPage(
  ...
  if (options?.description) {
    bodyLines.push('', '## Description', '', options.description);
  }
  ...
  if (options?.relationships && options.relationships.length > 0) {
    ...
    for (const rel of options.relationships) {
      bodyLines.push(`- ${entity.name} ${rel.predicate} ${rel.object} — ${rel.evidence} (${rel.pages})`);
    }
  }
