...
export function writeEntityPage(
  ...
  if (options?.description) {
    fallbackBodyLines.push('', '## Description', '', options.description);
  }

  if (options?.relationships && options.relationships.length > 0) {
    fallbackBodyLines.push('', '## Relationships');
    for (const rel of options.relationships) {
      fallbackBodyLines.push(`- ${entity.name} ${rel.predicate} ${rel.object} — ${rel.evidence} (${rel.pages})`);
    }
  }
  ...
  const finalBody = body?.trim() ? body : `${fallbackBody}\n\n> **Fallback:** this entity page was generated by deterministic fallback because the LLM did not produce a body.`;
