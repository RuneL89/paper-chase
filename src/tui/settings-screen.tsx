...
const RECOMMENDATIONS: Record<'anthropic' | 'openai' | 'qwen', Partial<Record<SettingRow, string>>> = {
  anthropic: {
    ...
    modelJsonCorrector: 'Haiku — cheap; writes short JSON-repair instructions',
  },
  openai: {
    ...
    modelJsonCorrector: 'GPT-5.6 Luna — cheap; writes short JSON-repair instructions',
  },
  qwen: {
 ...
 * v1.4.0 multi-provider extension; Qwen extension 2026-08-04; per-step
 * provider selection v1.9.0, user directive 2026-08-17) and the "API Keys"
 * section (v1.5.0, user directive 2026-07-23).
 *
 * A Default Provider row (Anthropic / OpenAI / Qwen / custom) sits above the
 * model rows and defines the DEFAULT slot: the Default Model row's provider
 * and what "Same as default" resolves to for every other row. Since v1.9.0
 * each of the seven model rows (Default, Extractor, Synthesis Writer, DOX
 * Writer, Curation — Phase 14 §2.6, Cross-Wiki Bulk, Cross-Wiki Judgment —
 * Phase 24, JSON Corrector — Phase 16 v1.0.5) is a self-describing
 ...
 * OpenAI DOX, Sonnet Synthesis) can never desync.
 *
...
export function SettingsScreen({ onBack, onResult, workspace = '.' }: SettingsScreenProps) {
  ...
  const cycleModel = (row: SettingRow, delta: 1 | -1) => {
    setSettings((prev) => {
      const getCurrent = (): ModelSlot | null => {
        ...
        if (row === 'modelJsonCorrector') return prev.models.jsonCorrector ?? null;
        return null;
      };
      ...
      if (row === 'modelDefault') {
        ...
        } else if (row === 'modelJsonCorrector') {
          models.jsonCorrector = nextSlot;
        }
      }
      return { ...prev, models };
    });
  ...
  const slotForRow = (row: SettingRow): ModelSlot => {
    ...
    if (row === 'modelJsonCorrector') return settings.models.jsonCorrector ?? fallback;
    return fallback;
  };
  ...
  const rowToCallType = (row: SettingRow): string | undefined => {
    switch (row) {
      ...
      case 'modelJsonCorrector':
        return 'json-corrector';
      default:
        return undefined;
    }
  };
  ...
  const runModelTest = async (row: SettingRow) => {
    ...
    const apiKey = resolveApiKeyForTest(
      ...
      slot.provider.startsWith('custom:')
        ...
        : settings.apiKeys[slot.provider as 'anthropic' | 'openai' | 'qwen'],
      ...
      {isRawModeSupported ? (
        <Box flexDirection="column" marginTop={1}>
          ...
          <Box flexDirection="column" marginTop={1}>
            ...
            {renderModelRow('modelJsonCorrector', 'JSON Corrector Model', settings.models.jsonCorrector ?? null)}
          </Box>
        ...
        <Box flexDirection="column" marginTop={1}>
          ...
          <Text>JSON Corrector Model: {slotLabel(settings.models.jsonCorrector ?? null)}</Text>
          {recLine('modelJsonCorrector', settings.models.jsonCorrector ?? null)}
