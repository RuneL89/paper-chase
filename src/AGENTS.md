...
* `commands/ingest.ts` — `ingest(slug, { workspace?, pagesPerChunk?, extract?, onProgress?, extractChunkFn? })`: hash-skip unchanged PDFs, page-by-page extraction, whole-page chunking (default 5), document pages via gray-matter, source pages, state; Layer 2 since Phase 2: per-chunk Extractor run (default-on, `extract?: boolean` opt-out) saving `.state/extracted/<chunk-id>.json`, additive `IngestResult.extractions` counts; Phase 4: after materialization, runs `validateWiki()` and logs link/citation/schema health via `logValidation()`, additive `IngestResult.validation`; Phase 5: after validation, runs `writeDoxContracts()` to regenerate the `index.md` navigation contracts; `extractChunkFn` is an optional test-only injection for LLM-free `extract: true` runs
...
* `dox-writer.ts` — deterministic Layer 4: scans `wikis/<slug>/` (excluding `raw/` and `.state/`), builds a tree of folders and markdown files, and writes `index.md` navigation contracts for every folder plus the wiki root; no LLM calls
...
* Page types are declared in frontmatter; known built-in types are `entity`, `topic`, `document`, `source`, `raw`, and `index` (added in Phase 5)
