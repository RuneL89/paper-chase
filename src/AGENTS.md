...
* `tui/` — Ink TUI: `app.tsx` (screen router), `menu.tsx` (main menu + exported `resolveMenuSelection`), `init-screen.tsx` (Title + Workspace only; slug derived from Title via `slugify`), `ingest-screen.tsx`, `add-pdfs-screen.tsx`, `extractor-test-screen.tsx`, `entity-browser.tsx`, `topic-browser.tsx`, `dox-browser.tsx`, `validation-report-screen.tsx`, `test-screen.tsx`, `settings-screen.tsx`, `components/` (header, footer, spinner, error-box, success-box, tree-browser), `hooks/use-wiki-list.ts`, `hooks/use-wiki-details.ts`, `hooks/use-raw-contents.ts`, `hooks/use-document-chunks.ts`
...
* `commands/ingest.ts` — `ingest(slug, { workspace?, pagesPerChunk?, extract?, synthesis?, onProgress?, extractChunkFn?, synthesizeEntityFn?, synthesizeEntityPermissiveFn?, synthesizeTopicFn?, synthesizeTopicPermissiveFn?, synthesizeDocumentFn?, synthesizeDocumentPermissiveFn? })`: hash-skip unchanged PDFs, page-by-page extraction, whole-page chunking (default 5), document pages via gray-matter, source pages, state; Layer 2 since Phase 2: per-chunk Extractor run (default-on, `extract?: boolean` opt-out) saving `.state/extracted/<chunk-id>.json`, additive `IngestResult.extractions` counts; Phase 4: after materialization, runs `validateWiki()` and logs link/citation/schema health via `logValidation()`, additive `IngestResult.validation`; Phase 5: after validation, optionally runs the Synthesis Writer per entity, topic, and document page in that order (opt-in `synthesis?: boolean`, default false) and, before `writeDoxContracts()`, writes synthesized pages only when the preservation check passes; Phase 6: after synthesis, runs `writeDoxContracts()` to regenerate the `index.md` navigation contracts; `extractChunkFn` and the `synthesize*Fn` injections are optional test-only helpers for LLM-free runs
...
* `pages/topic-page.ts` — deterministic `topics/<subfolder>/<slug>.md` writer grouped by claim type (phase doc §2.2); Phase 5 additive: optional `entities` and `context` fields for topic synthesis
...
* `dox-writer.ts` — deterministic Layer 5 (Phase 6): scans `wikis/<slug>/` (excluding `raw/` and `.state/`), builds a tree of folders and markdown files, and writes `index.md` navigation contracts for every folder plus the wiki root; no LLM calls
* `validation/link-checker.ts` — Phase 4: deterministic `[[Page Title]]` wikilink check; reports broken links and orphaned pages (excludes `index.md`, `sources/*.md`, and the wiki constitution `AGENTS.md`); Phase 5 additive: folder indexes resolve as `[[Folder Name]]` so DOX navigation links work
...
* `agents/` — Phase 2 landed `extractor.ts`; Phase 5 adds `synthesis.ts` (the Synthesis Writer, Layer 4); the Materializer (Phase 3) is deterministic code in `src/materializer.ts`, not an agent in this folder; DOX Writer code may not land before Phase 6
...
* The Extractor is the ONLY LLM call in the pipeline (vision `04` §1); every other concern (folder creation, page writing, memory updates, index contract generation) is deterministic code or a later phase
