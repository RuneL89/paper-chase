...
* `tui/` — Ink TUI: `app.tsx` (screen router), `menu.tsx` (main menu + exported `resolveMenuSelection`), `init-screen.tsx`, `ingest-screen.tsx`, `add-pdfs-screen.tsx`, `extractor-test-screen.tsx`, `entity-browser.tsx`, `topic-browser.tsx`, `dox-browser.tsx`, `validation-report-screen.tsx`, `test-screen.tsx`, `settings-screen.tsx`, `components/` (header, footer, spinner, error-box, success-box, tree-browser), `hooks/use-wiki-list.ts`, `hooks/use-wiki-details.ts`, `hooks/use-raw-contents.ts`, `hooks/use-document-chunks.ts`
...
* `validation/preservation-check.ts` — Phase 5: `checkPreservation` (entity), `checkTopicPreservation` (topic), and `checkDocumentPreservation` (document) verify synthesis did not drop any mention context, relationship evidence, claim text, extracted text, or existing source citation
* `agents/synthesis.ts` — Phase 5: the Synthesis Writer (Layer 4). `writeEntitySynthesis`, `writeTopicSynthesis`, and `writeDocumentSynthesis` each read their type-specific prompt (`synthesis.prompt.txt`, `synthesis-topic.prompt.txt`, `synthesis-document.prompt.txt`), inject page data + wiki constitution, and call `callLLM(prompt)` with 8192 max output tokens; output is a two-layer markdown article with synthesis at the top and preserved detail sections below. Additive permissive fallbacks: `writePermissiveEntitySynthesis`, `writePermissiveTopicSynthesis`, `writePermissiveDocumentSynthesis` read the corresponding permissive prompts and allow the model to summarize in Layer 1 while preserving the exact structured data in Layer 2; used by `ingest.ts` when strict synthesis fails preservation
