...
* `<slug>/` — one generated wiki: `raw/` (source PDFs, never modified), `documents/` (raw extracted chunks), `sources/` (provenance pages), `entities/`, `topics/`, `.state/` (tooling state, e.g. `ingestion.json`, `language.json` — the Phase 7 per-wiki language record: `outputLanguage`, `lastInputLanguage`; Phase 14 adds `curation-report.json` — the per-run topic/entity curation outcome incl. fallbacks, vetoes, deletions, and rewritten links — and `curation-overrides.json` — the user-editable `{"neverMerge": [["slug-a","slug-b"]]}` veto list, created empty on first curation run; Phase 16 adds `synthesis-state.json` — the per-page synthesis completion memory `{ "pages": { "<wiki-rel path>": { "mode", "dataHash", "synthesizedAt" } } }` behind resume-skipping and Materializer byte-preservation), and a generated `AGENTS.md` constitution (from `templates/AGENTS.md`; binding for every LLM call in that wiki, managed by the CLI — never hand-edited; includes the wiki's `## Language` section since Phase 7)
