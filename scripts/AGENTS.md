...
* `create-golden-master.ts` and `create-golden-master-da.ts` must **never be re-run** against the committed golden masters — the PDFs are immutable (see `test-pdfs/AGENTS.md`); the scripts exist to document how they were made
