...
**Worker-process isolation (amended 2026-09-02, user-ratified):** the per-PDF loop may run each PDF's mini-pipeline inside a dedicated child worker process — one worker per PDF, spawned and sequenced by the TUI/CLI parent (the conductor); the deferred tail (DOX Writer, workspace pass, Cross-Wiki Discovery, AGENTS.md Updater) runs in its own worker exactly once after the loop. Pipeline mechanics, sequencing, and concurrency law are unchanged: the pool cap 4 stays inside each worker, workers run strictly one at a time, and state-file writes stay serialized. The worker boundary exists for crash containment and memory hygiene: a worker that dies for ANY reason (escaped error, out-of-memory abort, native crash) never ends the run — the conductor captures the worker's stderr, appends a record to `.state/crash-log.jsonl`, and offers recovery: **retry** the same PDF (resuming from the per-PDF/per-page checkpoints, law unchanged) or **defer** the PDF to a future run — deferral is user-initiated only, deletes nothing, and never happens automatically. When auto-retry is enabled, the conductor retries with backoff up to a fixed cap (3 retries per PDF, 30-second backoff), then stops and surfaces loudly — it never auto-defers. The run-level outage response becomes this circuit breaker; the per-stage outage detector (§6) keeps its scope inside each worker. User-facing screens stay byte-identical in normal operation: worker progress is relayed to the same progress channel; the only new UI is the crash-recovery panel. **(Rider amended 2026-09-03, user-ratified — worker-scope fencing + conductor observability, born from the first production conductor run: the Phase 8 removed-PDF check read the worker's `onlyPdfs`-filtered file list and false-flagged every other recorded source, and the 2026-07-21 all-skipped repair fallback re-ran once per hash-skipped worker — 8h53m of redundant whole-wiki curation on the first skip-worker.)** Three laws: (1) the removed-PDF check is RUN-level — it runs only in unscoped runs, and the finalize worker (no selector, complete raw/ list) performs it exactly once per run; a scoped worker whose target vanished mid-run skips gracefully with one honest line. (2) The all-skipped repair fallback is RUN-level — per-PDF workers never run it; the conductor passes `idleFallback` to the finalize worker iff NO PDF was ingested that run, so an all-skip run still performs exactly ONE repair pass (the 2026-07-21 law, batch semantics restored). (3) The 2026-09-02 "only new UI is the crash-recovery panel" clause is amended to sanction three observability additions: per-worker banner lines on the progress channel (`[2/37] AKDB_2025.pdf`, `[finalize] …`), a persistent worker-position row (current worker, position, elapsed), and stall lines that name the failing call and render a live countdown (zero-clamped to `retry in flight…`); the stall-line wording of §6 gains the call label.

**Fine-grained crash resume (rider amended 2026-09-07, user-ratified — backlog B25, implemented by Phase 28; born from the 2026-09-04/05 production crashes: two workers died mid-extraction — exit 1 after extractor output-token-cap bursts — and each auto-retried worker restarted its PDF from chunk 001, re-paying ~5–8 min per already-completed chunk):** the retry clause above ("resuming from the per-PDF/per-page checkpoints") is extended to the finest completed granularity of the dead PDF's in-flight mini-pipeline. (1) **Per-chunk extraction checkpoints:** each chunk's `.state/extracted/<chunk-id>.json` carries its source provenance (PDF sha256 + page-range); a retry skips the Extractor for a chunk whose stored JSON parses, deterministically re-validates against the Extractor schema (no LLM call), and whose provenance matches the current PDF hash and chunk range — a changed-PDF re-ingest still deletes stale extraction JSONs first (and never deletes a fresh-hash checkpoint), so a hash-mismatched file can never be consumed. (2) **Per-PDF stage markers:** a small `.state/pdf-progress.json` records the in-flight PDF's completed stages (extraction; materialize/curation, keyed to an extracted-set fingerprint) so a retry enters at the right stage instead of re-running deterministic work whose inputs are unchanged. (3) **In-flight synthesis journal:** the progress file's synthesis section records a stage cursor (entities → topics → composites → comparisons) with per-page done/queue, so a crash mid-synthesis resumes at the exact page; the per-page synthesis records (Step 9) remain the authoritative durable skip law, and the whole progress entry is deleted when the PDF's own ingestion record lands (Step 11). Coverage stops at the end of each PDF's mini-pipeline — the finalize worker's deferred tail stays a re-runnable unit. **Data is never skipped:** a valid on-disk extraction is data, not skipped work; any invalid, stale, or fingerprint-mismatched checkpoint is conservatively recomputed. **Crash observability (same rider):** the worker's caught fatal error/stack is captured into the crash record (additive `fatalError`/`fatalStack` fields) and mirrored to worker stderr, and the crash-log stderr tail filters LLM cost lines so real signal survives the tail cut.
...
**Checkpointing (amended 2026-07-25, user-ratified; fine-grained resume extended 2026-09-07, user-ratified — §1 rider, Phase 28):** state is not only written at the end of the run. Each PDF is recorded as ingested in `.state/ingestion.json` as soon as its own processing completes, so an aborted run never re-extracts finished PDFs; per-page synthesis records (Step 9) are likewise written as pages complete. The 2026-09-07 extension carries the same guarantee INSIDE an in-flight PDF: completed chunks (provenance-validated extraction JSONs), completed stages (materialize/curation, keyed to an extracted-set fingerprint), and completed synthesis pages (the Step 9 records plus the in-flight journal cursor) are never repeated after a crash — a retry enters exactly where the dead worker stopped, through synthesis/amendment. The end-of-run write remains the final, complete record.
...
- **Transient transport failures** (HTTP 429/5xx, network errors, timeouts) are retried with escalating stall waits when the caller has opted into retries, each failed attempt logged to `.state/transport-stalls.jsonl` and surfaced live on the ingest progress channel — cheap completion over speed, data never skipped (amended 2026-08-20/22 for 429/5xx and **2026-08-30 for network/timeout**, each user-ratified; the original 2026-07-20 ≤3-attempt bound no longer governs any transient class for opted-in callers): an HTTP **429 or 5xx** gets 6 total attempts with a 1/5/15/45/90-minute stall floor (a provider `Retry-After` is honored but clamped to 90 minutes); a **network/timeout** error (DNS, socket, headers/body timeout, per-attempt absolute deadline) gets the same 6 total attempts with its own **10/20/30/60/90-minute** ladder — a silently-dead or slow connection is ridden out with visible, audited retries instead of aborting, and exhaustion after the ladder still fails loud. `maxRetries: 0` keeps the frozen no-retry default for every class. **Per-page transport fallback (amended 2026-07-25, user-ratified):** when a transient failure is still throwing after the bounded retries at a **per-page stage** (entity or topic synthesis), the error is caught for THAT page and the page falls back to the deterministic structured template — one hiccup costs one page's prose, never the run. The fallback is logged loudly and counted in metrics. **Outage detector:** the abort returns when transport-failed pages reach **5 consecutively** (no successful call in between) or **more than 10% of a stage's pages** — that signature means a real outage, and the run aborts with the transport error (fail loud). HTTP 4xx NEVER gets a per-page fallback — a configuration problem must not silently template a whole wiki. Other stages are unchanged: the Extractor's thrown errors still abort (its per-chunk fail-loud), and DOX/workspace/updater already catch thrown errors once per target into their deterministic fallbacks.
