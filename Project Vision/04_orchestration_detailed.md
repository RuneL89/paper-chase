...
7. Run the workspace pass — the topmost step of the same bottom-up chain (folder indexes → wiki root index → workspace index): read the triggering wiki's freshly-written root `index.md` (the pass's only content input — never the wikis' content pages) and call the LLM once to write that wiki's own contribution to `wikis/index-of-indexes.md`, in this run's output language (§9). The file is then re-composed deterministically: the triggering wiki's prose segment and `## Wikis` catalog line are replaced, every other wiki's prose and lines are preserved byte-for-byte (amended 2026-07-21, user-ratified per-wiki segments — a wiki's DOX Writer never rewrites another wiki's contribution), and deterministic code re-imposes the children list, statistics, and section framing over all wikis.
