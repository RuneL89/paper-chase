...
**2026-08-17 supersession note (user directive, recorded as root AGENTS.md preference 2026-07-22):** per-step provider + model routing now lets every slot carry its own provider (`{ provider, model }` pairs), so the table above is GUIDANCE, not a constraint — a slot the user sets explicitly follows the user's choice (for example, a cheaper provider for the Extractor, a stronger prose provider for Synthesis, and a mid-tier provider for the DOX Writer). Custom providers (`custom:<id>`) can be added in Settings when needed.
