...
**2026-08-17 supersession note (user directive, recorded as root AGENTS.md preference 2026-08-17):** per-step provider + model routing now lets every slot carry its own provider (`{ provider, model }` pairs), so this table is GUIDANCE, not a constraint — a slot the user sets explicitly follows the user's choice (the user's reference table: Qwen for the Extractor, DeepSeek-V4-Pro for the DOX Writer, Anthropic Sonnet for the Synthesis Writer). DeepSeek carries a single model (`deepseek-v4-pro`, peak $1.32/$3.96) for now, so its slots have nothing to recommend between; recommendation labels only render for providers with a tier choice. **2026-08-19 addition:** the Zhipu provider (international Z.ai) joins the guidance with a free cheap tier — `glm-4.7-flash` ($0/$0; 1-request free-tier concurrency, so sequential steps only) as its Extractor/Cross-Wiki-Bulk pick, `glm-5.2` as mid-tier, and `glm-5.3` (mandatory reasoning) as the premium.
