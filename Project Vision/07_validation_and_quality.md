...
- **Transient transport failures** (HTTP 429/5xx, network errors, timeouts) are retried with escalating stall waits when the caller has opted into retries, each failed attempt logged and surfaced live (amended 2026-08-20/22 for 429/5xx and 2026-08-30 for network/timeout, each user-ratified; supersedes the 2026-07-20 ≤3-attempt bound for opted-in callers): 429/5xx get 6 total attempts with a 1/5/15/45/90-minute stall floor (`Retry-After` honored, clamped to 90 minutes); network/timeout errors get the same 6 total attempts with a 10/20/30/60/90-minute ladder — a silently-dead or slow connection is ridden out with visible, audited retries instead of aborting, and exhaustion still fails loud. `maxRetries: 0` keeps the frozen no-retry default. At the per-page synthesis stages, a transport failure still throwing after those retries falls back per page to the deterministic structured template (logged loudly, counted in metrics — amended 2026-07-25, user-ratified), and the run aborts only when the outage detector fires: 5 consecutive transport-failed pages, or more than 10% of a stage's pages. HTTP 4xx never falls back per page — it aborts immediately, as a configuration problem must never silently template a wiki.
