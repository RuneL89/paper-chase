...
6. **generic-label disambiguation** (added 2026-08-26, user-ratified — see below).
...
**Class 6 — generic-label disambiguation (amended 2026-08-26, user-ratified):** a generic label that different sources use for **different real things** — the observed case: `Indikator 2` means "first treatment contact within 10 days" in one registry's report and "surgery within 24 hours" in another's — must never be silently conflated into one homogeneous page. The identity rule is semantic, not lexical: same wording with different meanings is NOT one entity/topic, exactly as different wording with the same meaning IS one (the §merge rule). When the Materializer detects a generic-pattern slug whose per-source evidence has low substantive overlap, an LLM heterogeneity judgment (fed the per-source sample claims/significance) decides `split` or `no-split`; a confirmed split renders ONE composite page at the generic slug with one member per distinct meaning, evidence grouped per member under its own subheading. A label that means the same thing across sources (the same register's indicator across report years) stays one ordinary page — the mechanism fires only on heterogeneity, never on the generic pattern alone. Unlike classes 1-5 this class applies to **topics as well as entities** (a claim-type slug like `indicator-2` collides the same way an entity slug does); disambiguation is the ONE sanctioned topic-composite class. Detection, judgment, application, and the sticky record are specified in `04_orchestration_detailed.md` §3.2 Step 6b.
