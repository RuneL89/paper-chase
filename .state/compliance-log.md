...
[2026-08-22 15:00] Phase 16 v1.0.4 — persistent 429/5xx stall audit log (user directive 2026-08-22)
  Changed: src/llm/client.ts, tests/phase-16.test.ts, tests/AGENTS.md, src/AGENTS.md, wikis/AGENTS.md, root AGENTS.md, Implementation Plan/PHASE_16_run_resilience.md, scripts/launcher-entry.ts (VERSION 1.0.20 -> 1.0.21)
  Reason: The v1.0.3 stall ladder rides 429/5xx out over up to ~2.6 h, but nothing persisted the failures — the live progress lines scroll by and a post-mortem is blind. User directive 2026-08-22: every 429/500 error and its retries must be visible in the relevant logs for later monitoring and auditing.
  Behavior: every failed 429/5xx attempt is appended to the wiki's `.state/transport-stalls.jsonl` (beside `llm-calls.json`) — one JSON record per attempt: timestamp, callType/context, provider, model, statusCode, attempt (1-based failed attempt), maxAttempts, exhausted (true on the final failed attempt), waitSeconds (the stall before the next attempt; 0 when exhausted), and the provider's error message. Writes funnel through the same serialized write queue (pool-safe), are best-effort, and never carry key material. Kept OUT of `llm-calls.json` deliberately: that log is one-entry-per-successful-call and feeds the cost sums and the repair-rate denominator — stall records must not perturb them. Network errors are unchanged (console.warn only, caller <=3-attempt bound).
  Test touches (enumerated): phase-16 NEW gate 16.18 (client-level: 429-then-success -> exactly one stall record [attempt 1, 60s wait] + success-only call log; persistent 500 -> six records [attempts 1..6, waits 60/300/900/2700/5400/0, last exhausted with the provider message] and no call log); phase-16 gate 16.17 ingest-level leg gains the workspace `.state/transport-stalls.jsonl` existence assertion.
  Tests: npx tsc --noEmit clean; key-less full suite 519 passed + 14 self-skipped, 0 failed (34 files passed, 1 skipped). Acceptance run key-less (`.env` moved to `.env.keyless-tmp` and restored byte-identical).
  Checked By: Main agent

[2026-08-23] Phase 16 v1.0.5 — JSON-corrector recovery loop + transport hardening (user-ratified plan 2026-08-23)
  Changed: src/llm/json-corrector.ts (NEW), prompts/json-corrector.prompt.txt (NEW), src/llm/reask.ts, src/llm/client.ts, src/agents/extractor.ts, src/agents/curation.ts, src/cross-wiki/llm.ts, src/tui/settings.ts, src/tui/settings-screen.tsx, tests/phase-12.test.ts, tests/phase-16.test.ts, tests/phase-11.test.ts, tests/phase-14.test.ts, AGENTS.md (root), src/AGENTS.md, prompts/AGENTS.md, tests/AGENTS.md, scripts/launcher-entry.ts (VERSION 1.0.21 -> 1.0.22)
  Origin: the 2026-08-22/23 rkkp-dhhd ingest failures — (1) DHHD_2024 part-023 exhausted its 3 reasks and aborted the ingest: two 32,768-token truncations then "Bad control character in string literal in JSON at position 7292" on the final attempt; the generic correction block could not teach the free GLM model WHAT broke. (2) The restarted run froze ~9.5 h overnight with zero CPU, zero open sockets, zero log lines: the stall ladder reacts to RETURNED failures only, and undici's built-in timeouts are activity-based, so a silently-dead connection hung the request forever (the same mode froze the 2026-08-23 morning run on part-003).
  Behavior (a) — JSON corrector: a JSON PARSE failure at any JSON-producing stage (Extractor, Curation, cross-wiki JSON calls) fires an LLM diagnostician (callType 'json-corrector', log context '<ctx>#json-diagnosis', maxRetries 1, 2048-token cap, temperature 0) over the parse error + a BOUNDED evidence excerpt (full output <=4000 chars; +/-800-char window around a named error position; head+tail 1500-char excerpts otherwise); its targeted instruction joins the reask correction block as a 'Diagnosed cause and fix:' section (buildCorrectionBlock's optional enhancement param). The provider's stop reason reaches validators via the additive CallLLMOptions.onResponseMeta tap ('length'/'max_tokens' = truncation), and truncated outputs echo only their TAIL into the block so the repair call does not approach the same token ceiling. The corrector is ADVISORY (any corrector-side failure degrades to the plain block), never fires for schema/rule failures, and rides the real transport only — the callLLMFn test seams bypass it, so the LLM-free gates stay $0 even with a key present. The <=3-attempt content-defect budget, fail-loud Extractor exhaustion, curation keep-all fallback, and cross-wiki never-abort posture are UNCHANGED; data is still never skipped. Synthesis/DOX/Updater are deliberately out of scope (markdown outputs with already-verbatim-targeted validators). Routing: the additive jsonCorrector slot (models.jsonCorrector in .paper-chase.json, legacy -> null -> Default-slot fallback) surfaced as the 'JSON Corrector Model' Settings row (the eighth per-step row; cheap-tier recommendation labels).
  Behavior (b) — absolute per-attempt deadline: every LLM attempt carries AbortSignal.timeout (LARGE_CALL_DEADLINE_MS 900_000 when maxTokens >= 32768, else CALL_DEADLINE_MS 300_000; the Settings test probe 30s). A timed-out attempt throws into the existing network-error path (the caller's <=3-attempt bound with the 5/15/45s backoff), converting an unbounded hang into a fast loud retry.
  Behavior (c) — Retry-After clamp: the honored 429 Retry-After header is parsed in BOTH forms (parseRetryAfterSeconds — delta-seconds and HTTP-date) and clamped to RETRY_AFTER_MAX_SECONDS = 5400 (the 90-minute top stall rung). AMENDS the 2026-08-20 honor-Retry-After preference: delay = max(exponential, floor, min(header, 90 min)) — recorded as a preference amendment in root AGENTS.md.
  Test touches (enumerated): phase-12 gate 12.1 RE-CAST to the corrector-aware flow (3 spy calls: extractor -> json-corrector -> extractor attempt 2; intent preserved: correction block carries the parse error + invalid output verbatim, now plus the diagnosis section); NEW phase-12 gates 12.9 (degraded-corrector fallback; truncation NOTE + tail-only echo; schema-no-corrector; helper units — async validator, enhancer block shape, throwing enhancer, bounded-evidence/echo helpers), 12.10 (curation leg + callLLMFn bypass), 12.11 (cross-wiki leg + callLLMFn bypass); NEW phase-16 gates 16.19 (AbortSignal on every attempt, large-call headersTimeout preserved, TimeoutError -> bounded network class), 16.20 (Retry-After clamp + HTTP-date parsing), 16.21 (onResponseMeta anthropic/openai/end_turn legs); NEW phase-11 gate 11.18 (slot seed/normalize/resolve/persist + Settings row render); phase-11 gates 11.1/11.10/11.11/11.13/11.14/11.15/11.17 DOWN-count walks +1 for the new row and seed-shape toEquals gain jsonCorrector: null; phase-14 gate 14.10 seed shapes likewise (assertion intents unchanged).
  Tests: npx tsc --noEmit clean; key-less full suite 531 passed + 14 self-skipped, 0 failed (34 files passed, 1 skipped). Acceptance run key-less (.env moved to .env.keyless-tmp and restored byte-identical: 171 bytes, mtime 2026-07-20 00:13). One flake observed and cleared: phase-12 gate 12.5 failed once mid-run (rootCalls 0 vs 2) under full-suite load and passed on the two subsequent full runs — the documented timing-flakiness posture (untouched code path).
  Checked By: Main agent

[2026-08-26] Removed DeepSeek/Zhipu/GLM built-in providers (user directive: push everything to main except the inclusion of the new LLM providers)
  Changed: src/llm/client.ts (Provider union narrowed to anthropic/openai/qwen/custom; removed DeepSeek/Zhipu endpoints, env vars, price-table entries, and dispatch branches), src/tui/settings.ts (BuiltInProvider narrowed; removed MODEL_CATALOG/deepseek and /zhipu, default/curation entries, and apiKeys fields), src/tui/settings-screen.tsx (removed Default Provider cycle entries, API-key rows, recommendation labels, and provider-specific casts for the two removed providers), tests/phase-11.test.ts (deleted gates 11.15 and 11.17; rewrote gate 11.16 and apiKeys expectations to use Anthropic/OpenAI/Qwen only; recalculated row-count navigation), tests/phase-16.test.ts (generalized the HTTP-500 mock comment), AGENTS.md (root), src/AGENTS.md, tests/AGENTS.md, Implementation Plan/PHASE_11_polish.md (version 1.10.0 -> 1.9.0; removed v1.10.0 Zhipu amendment and all DeepSeek/Zhipu/GLM gates/UATs), Implementation Plan/PHASE_16_run_resilience.md (generalized free-tier examples), scripts/launcher-entry.ts (asset VERSION 1.0.23 -> 1.0.24), dist/chase.mjs, dist/launcher.cjs, dist/paper-chase.exe (rebuilt)
  Behavior: The per-step provider/model routing feature is preserved; only the DeepSeek and Zhipu built-in provider catalogs and API-key rows are removed. Custom providers (`custom:<id>`) remain available. The 429/5xx stall ladder, transport-stalls.jsonl audit log, JSON corrector, per-attempt absolute deadline, and Retry-After clamp are all unchanged.
  Tests: npx tsc --noEmit clean; key-less full suite 533 passed + 14 self-skipped, 0 failed (35 test files passed, 1 skipped, 36 files discovered including the four pre-existing untracked .state/upgrade/*.test.mts debug scripts). Acceptance run key-less (.env moved to .env.keyless-tmp and restored byte-identical). The packaged dist/paper-chase.exe rebuilt at asset VERSION 1.0.24; a grep of dist/chase.mjs / dist/launcher.cjs found no deepseek/zhipu/glm literals.
  Checked By: Main agent

  Reason: User asked to change the paper-chase.exe icon (dist/). Requirements honed via ask-questions: I design the icon (no source asset existed); paper-chase.exe only (runtime-node.exe and the debug raw exe stay uniconed); baked into the build, not a one-off patch. Design approved by the user from rendered previews: paper stack -> wiki link-graph concept, terminal-green on a dark rounded plate.
  Behavior: scripts/build-icon.ts renders the two SVGs to exact-size RGBA PNGs via headless Chrome/Edge screenshots and packs the 16/24/32/48/64/128/256 multi-size assets/icon.ico with a dependency-free packer (this build machine has no standalone npm on PATH, so no sharp/png-to-ico/@electron/rcedit devDeps — see assets/AGENTS.md). scripts/set-icon.ts is the new packaging step 3 (appended to package:win) and patches the icon with the vendored electron/rcedit v2.0.0 (scripts/vendor/rcedit-x64.exe). CRITICAL finding: a bare `rcedit --set-icon` BREAKS the exe — rcedit's PE rewrite drops pkg's 132,722,376-byte payload overlay (the snapshot with all extracted assets), and pkg bakes PAYLOAD_POSITION (its own section-table end) into the prelude at build time, so the payload must be restored at the ORIGINAL offset. set-icon.ts therefore captures the overlay, runs rcedit, zero-pads the section-table gap rcedit's re-packing leaves, and re-appends the overlay at the baked position (final size == fresh pkg output, 190,163,144 bytes); it refuses loudly if rcedit grows the sections past the baked position. A mis-placed payload boots degraded ("Pkg: Error reading from file.") and the launcher "extracts" LIVE PROJECT FILES instead of the snapshot — the first build did exactly that, which is how the mis-placement was caught (the degraded extraction is byte-perfect but reads the repo, not the snapshot). No VERSION bump: the icon is a PE resource, not a snapshot asset, so the launcher's marker guard and extraction set are untouched.
  Verified: full pipeline rebuild (build-exe -> build-launcher -> pkg -> set-icon) on dist/runtime-node.exe; final exe first-run extracts cleanly with NO pkg error and prints 1.0.0 (exit 0); the extracted snapshot is byte-identical to the sources (chase.mjs, pdf.worker.mjs, node.exe, templates/AGENTS.md, all 24 prompts, 16 pdfjs fonts, canvas binaries); ExtractAssociatedIcon returns the 32 px frame pixel-identical to the packed PNG (0/1024 mismatches).
  Checked By: Main agent

[2026-08-26] Canon pass — Generic-label disambiguation (Option E Variant B) + Per-PDF patch amendment (Option B Patch); phases 25/26 canonized, NO implementation (user directive: "canon ... up until implementation")
  Changed (documents only — zero code): Project Vision/02_WIKI_concept_detailed.md (§4.6: rollup classes five → enumerated set, class 6 added; the class-6 mechanism paragraph), Project Vision/05_page_types_specification.md (§6: class 6 + the disambiguate paragraph, class range 1-6; §7: the same-label-different-meaning topic rule), Project Vision/04_orchestration_detailed.md (§1: per-PDF sequential ingestion paragraph; §3.2: loop-scope note, Step 6 per-PDF cadence, NEW Step 6b disambiguation pass, Step 9 amendment-synthesis patch-output paragraph; §4: amendment mode + patched-page preservation pointer; §6: patch outputs added to the quality-failure class with full-synthesis fallback), Project Vision/07_validation_and_quality.md (§3: patched-page preservation over the MERGED page; §5: patch quality-failure sentence), Project Vision/01_PRODUCT_VISION_AND_ARCHITECTURE.md (§4.1: curation per-PDF cadence), Implementation Plan/PHASE_25_generic_label_disambiguation.md (NEW), Implementation Plan/PHASE_26_per_pdf_patch_amendment.md (NEW), Implementation Plan/IMPLEMENTATION_PLAN_MASTER_INDEX.md (phase directory + vision mapping + token budgets + "What You Have" + files list + overview range + time estimate), Implementation Plan/BACKLOG.md (B23 → SCHEDULED Phase 25 with its open decisions resolved; NEW B24 → SCHEDULED Phase 26), Implementation Plan/AGENTS.md (phase range 11–26 + unimplemented note), Project Vision/AGENTS.md (04 ownership line), AGENTS.md (root — 2026-08-26 User Preferences entry + Implementation Plan child-index line).
  User decisions (2026-08-26, AskUserQuestion round + session directives): (1) per-PDF loop is the NEW DEFAULT (batch pipeline removed, not kept behind a flag); (2) patch output ONLY from day one (no full-page-amendment intermediate); (3) curation runs PER PDF inside the loop (identity settled before each PDF's synthesis; ~2 extra calls per PDF accepted); (4) Phase 25 (E Variant B) BEFORE Phase 26 (B Patch) — patches need stable member-scoped targets. Earlier same-day decisions: Option E Variant B (composite at the generic slug, not child pages); "Decided" backlog entry B23.
  Contradiction Protocol (pre-check): seven collisions with prior canon found, ALL user-resolved in this pass — 02 §4.6/05 §6 "five classes only" (→ six), Phase 22 "topics never cluster" (→ class-6 exception), 04 §3.2 batch materialize order (→ per-PDF loop), 04 §1/"per-ingest" curation cadence (→ per-PDF), 04 Step 9 changed-fingerprint ⇒ full re-synthesis (→ patch amendment when a same-shape synthesis exists), 07 §3 preservation scope (→ merged-page union check), 04 §6 quality-failure vocabulary (→ patch outputs). No unresolved contradictions.
  Boundary honored: vision amendments and phase documents only; no src/, tests/, prompts/, or template changes; asset version untouched; both phase docs marked "Status: Planned (canonized 2026-08-26 — not yet implemented)".
  Checked By: Main agent


[2026-08-27] Phases 25/26 — LLM budget relaxation + glm-5.3-flash-only constraint (user directive: "I'm ok with having some LLM cost - as long as you use the glm-5.3-flash model only and you outline what you need to run through llm - including tests and UAT verification")
  Changed (documents only — zero code): Implementation Plan/PHASE_25_generic_label_disambiguation.md (budget line $0 → $2.00 hard cap glm-5.3-flash-only; NEW §3 "LLM Call Inventory" — the one new `disambiguate` call type + the existing call types fired by live gate/UAT runs, each with per-call token/cost estimates and live-touch counts incl. Verifier re-runs; NEW live Gate 25.9 — real disambiguate() against glm-5.3-flash, schema-valid-JSON assertion, llm-calls.json provider/model assertion, self-skips without ZAI_API_KEY; sections renumbered 3→4/4→5/5→6/6→7; checklist 8→9 gates; UAT section gains the slot-pinning rule + per-UAT spend estimates), Implementation Plan/PHASE_26_per_pdf_patch_amendment.md (budget line → $5.00 hard cap glm-5.3-flash-only; NEW §3 LLM Call Inventory — the one new `synthesis-amend` call type + extractor/curation-per-PDF/synthesis/DOX/disambiguate fired by the UAT runs incl. the abort/resume re-extraction semantics (checkpoints are per PDF, so a mid-PDF abort re-extracts that PDF's chunks) and the DOX-runs-once-total guarantee for UAT 26.2; NEW live Gate 26.11 — real Amendment Writer end-to-end (patch schema-valid + merged page passes preservation; one reask allowed; exhaustion-into-full-synthesis FAILS the gate since it must prove the happy path); sections renumbered; checklist 10→11 gates; UAT slot-pinning + spend estimates), Implementation Plan/IMPLEMENTATION_PLAN_MASTER_INDEX.md (phase-directory LLM Cost column + token-budget table rows for 25/26), AGENTS.md (root — 2026-08-27 User Preferences entry).
  Model-pin enforcement: the dedicated UAT workspace's .paper-chase.json sets ALL routing slots to { provider: 'zhipu', model: 'glm-5.3-flash' } so no call in these tests can reach another model; the amendment call rides the synthesis slot and disambiguation rides the curation slot, so slot pinning covers both new call types.
  Expected actual spend: Phase 25 <$0.50 (gate ≈ $0.002, each live UAT ingest ≈ $0.05-0.25); Phase 26 <$1 (gate ≈ $0.004, UAT 26.1 ≈ $0.10-0.30, UAT 26.2 abort+resume ≈ $0.15-0.40). Caps deliberately generous for reask loops and resume re-fires. glm-5.3-flash is the PAID flash tier ($0.15/$0.50 per MTok — normal concurrency, pool cap 4 safe; throttling rides the 429/5xx stall ladder).
  Key-less guarantee unchanged: every live gate self-skips without ZAI_API_KEY; all other gates remain stub-based.
  Checked By: Main agent


[2026-08-27] Phases 25/26 — LLM budgets TRIPLED by user directive ("Let's triple the llm budget")
  Changed (documents only — zero code): Implementation Plan/PHASE_25_generic_label_disambiguation.md (budget $2.00 → $6.00, 80% pause $1.60 → $4.80, cap references in the LLM Call Inventory updated), Implementation Plan/PHASE_26_per_pdf_patch_amendment.md ($5.00 → $15.00, pause $4.00 → $12.00, inventory cap reference updated), Implementation Plan/IMPLEMENTATION_PLAN_MASTER_INDEX.md (phase-directory cost column ≤$2/≤$5 → ≤$6/≤$15 + token-budget table rows), AGENTS.md (root — the 2026-08-27 preference entry's cap figures).
  Unchanged: glm-5.3-flash-only pinning, the LLM Call Inventories, gates 25.9/26.11, expected actual spend estimates (<$0.50 / <$1 — the caps are ceilings, not expectations), key-less self-skip posture.
  Checked By: Main agent


[2026-08-27] glm-5.3-flash key verification (user directive: "Verify the api key works for glm-5.3-flash before I tell you to start")
  Method: two live probes against https://api.z.ai/api/paas/v4/chat/completions using the zhipu key stored in dist/.paper-chase.json (the packaged-exe workspace config; untracked, key never printed).
  Results: probe 1 (max_tokens 16): HTTP 200 in 1.6s, model glm-5.3-flash accepted, content EMPTY with finish_reason length — all 16 completion tokens were hidden reasoning_tokens (15). Probe 2 (max_tokens 512, temperature 0): HTTP 200 in 1.9s, finish stop, exact JSON {"ok": true}; 27 prompt / 55 completion tokens of which 48 reasoning. Combined cost ~$0.00004.
  Findings: (1) the dist-stored zhipu key is VALID for glm-5.3-flash; live gates 25.9/26.11 and the live UATs can run for real. (2) glm-5.3-flash is a REASONING model — hidden reasoning tokens are billed as output ($0.50/MTok) and precede visible content, so content-only output estimates undercount real billing ~2-3x and small max_tokens caps can return content-empty length-finished responses (a cap problem, not a model failure; Phase 16 v1.0.5's finish-reason truncation tap already classifies this correctly).
  Changed (documents only): PHASE_25 + PHASE_26 LLM Call Inventory sections gained the verified-live note + the reasoning-token caveat.
  Checked By: Main agent


[2026-08-27 02:00] Phase 25 Compliance Check (pre-implementation)
  Changed (planned): src/agents/pre-merge.ts, src/materializer.ts, src/agents/disambiguation.ts (NEW), src/state/curation-decisions.ts, src/pages/composite-page.ts, src/validation/schema-validator.ts, prompts/disambiguation.prompt.txt (NEW), tests/phase-25.test.ts (NEW), templates/AGENTS.md (class-6 note), scripts/launcher-entry.ts (asset VERSION bump), AGENTS.md chain (DOX pass)
  Vision Docs Checked: 02_WIKI_concept_detailed.md §4.6 (class-6 amendment), 05_page_types_specification.md §6 (class 6 + disambiguate mechanism) + §7 (same-label rule), 04_orchestration_detailed.md §3.2 Step 6b (the pass), 07_validation_and_quality.md §5 (curation-class failure posture)
  Sections Checked: class-6 detection gates (pattern + heterogeneity, both required); deterministic-proposes/LLM-confirms; one composite at the generic slug (members = per-meaning identities); topics allowed for class 6 ONLY; sticky disambiguate record with sourceMap + splits escape hatch; keep-one-page fallback; curation-slot routing (callType 'disambiguate'); per-member verbatim preservation.
  Result: COMPLIANT — the phase doc restates the vision amendments 1:1 (authored together 2026-08-26); no contradiction found. Implementation details not fixed by vision (regex alternation set, stopword list, overlap threshold) are phase-doc-level choices, checked by gates 25.1/25.4.
  Checked By: Main agent

[2026-08-27 07:50] Phase 25 — LIVE gate 25.9 run + two live-caught fixes (main agent; the Implementer sub-agent hit a 5-hour usage limit mid-fix, reset ~10:34)
  Changed: src/agents/disambiguation.ts (basename source-echo normalization in validateDisambiguationVerdict — canonical full-path form retained in the normalized verdict and the sticky sourceMap; ambiguous basenames deliberately not normalized; the Phase 17 B1 citation-checker precedent), src/llm/client.ts (post-loop parse dispatch: provider === 'openai' || 'qwen' || 'deepseek' || 'zhipu' → parseOpenAIResponse), tests/phase-25.test.ts (gate 25.2 basename-echo regression legs), tests/phase-16.test.ts (NEW gate 16.22 — the zhipu/deepseek empty-parse regression), tests/AGENTS.md (16.22 documented + verification-history entry), .state/phase-25-status.json.
  Root causes (both caught ONLY because gate 25.9 is the suite's first LIVE zhipu call): (1) the model echoes member "sources" as basenames ("DPD_2025.pdf") while proposals carry full workspace paths — the exact-match validator failed every attempt (validation-exhaustion); reproduction with the identical prompt via direct endpoint calls returns a perfectly valid verdict, isolating the defect to the validator. (2) src/llm/client.ts's post-loop response-parse dispatch listed only openai/qwen for parseOpenAIResponse, so zhipu/deepseek 2xx Chat-Completions bodies fell into parseAnthropicResponse → EMPTY text, 0/0 tokens, no throw — the model's valid output was silently discarded. The bug is in HEAD (present in 380e0ca) and affects EVERY zhipu/deepseek callLLM in the main path; the dist bundles are equally affected and must be rebuilt before any zhipu/deepseek production use.
  Result: gate 25.9 GREEN (both invocations schema-valid; llm-calls.json entry provider zhipu / model glm-5.3-flash / callType disambiguate); gate 16.22 green; npx tsc --noEmit clean; FULL key-less suite 548 passed + 15 skipped across 37 files (36 passed + 1 skipped), zero failures (.env stashed/restored byte-identical, 171 bytes).
  LLM spend: ~$0.006 of the $6.00 cap (9 discarded-parse diagnosis attempts + 2 successful gate calls + 4 direct probes, all glm-5.3-flash).
  Pending: the INDEPENDENT Verifier sub-agent pass (cold compliance check + independent re-runs) — deferred until the sub-agent usage limit resets; UATs 25.1-25.3 not yet run (they are the user's acceptance steps).
  Checked By: Main agent

[2026-08-27 16:30] Phase 25 — LIVE UATs 25.1 + 25.2 executed by the main agent (CLI runs in the project wikis/ folder, per user request; every routing slot of the root .paper-chase.json pinned to zhipu/glm-5.3-flash for the runs, zhipu key injected from dist/.paper-chase.json — root config restored byte-identical afterwards)
  UAT 25.1 (two-registry split, wikis/uat25-split — KEPT as the acceptance artifact):
    - Run 1 (template constitution): NO split fired — the DPD extractor dissolved indicators into claims while HOFTER emitted indicator entities; the detector's >=2-source gate correctly held. Honest finding: the split only fires when the extractor names the label consistently in both PDFs.
    - Run 2 (constitution rule "extract every numbered indicator as its own entity, slug indikator-N"): 9 colliding labels judged and split (indikator-1/2/5/6/12/13/14/15/17) — 9 disambiguate calls, $0.011; indikator-2 -> members indikator-2-forste-behandlingskontakt (DPD_2025) + indikator-2-operation-senest-24-timer (HOFTER_2025) with an exact sourceMap; class-6 composite rendered at the generic slug with per-member evidence; the synthesized composite's lead names both meanings (reading check passed).
    - Run 3 (resume, after a ONE-OFF hang): 0 PDFs re-ingested, 0 pages re-synthesized, ZERO new disambiguate calls (sticky pre-application held live), DOX completed (17 index files), final validation PASSED. Resume $0.1334.
    - WATCH ITEM (new): run 2 hung ~1h between validation and the first DOX call — the last llm-calls entry's flush landed ~1h late, indicating a serialized-write queue backlog; the resume (same code) completed the same stage normally, so it is a non-reproduced race, not a deterministic hang. Recorded as a watch item, not a blocker.
  UAT 25.2 (same-register control):
    - Full-ingest control ATTEMPTED twice (DPD_2024 + DPD_2025 fixtures): blocked both times by glm-5.3-flash extraction degradation on the DPD_2024 chunk — 32K-truncation on attempts 1-2 + "headers timeout after 600000" on later attempts (the small-request probe stayed fast at 2.3s, so the API itself was healthy; the flash tier could not complete the large extraction prompt within the deadline at that hour). The Extractor's fail-loud exhaustion aborted the run exactly as designed (vision 04 §6).
    - Control proven at the mechanism level instead (wikis/uat25-control/.state/uat-25.2-control-evidence.json): (a) the REAL detector tokenizer computes Jaccard overlap 0.529 for the two-year same-meaning evidence (threshold 0.2 -> NO FLAG); (b) the REAL disambiguateLabel agent (live glm-5.3-flash, no stub) returned { split: false }, schema-valid, $0.0004. Detector does not flag + judgment says one meaning = one ordinary page — the control's semantic holds end-to-end.
  Spend: UAT 25.1 total $0.6578 (incl. resume) + control attempts ~$0.09 + control check $0.0004; cumulative Phase 25 live spend (gates + UATs) ~$0.75 of the $6.00 cap.
  Cleanup: uat25-control wiki (never produced pages) deleted; the scratch control script removed; .paper-chase.json restored from the pre-UAT backup (apiKeys anthropic/openai, default claude-haiku-4-5-20251001); wikis/uat25-split kept as the acceptance artifact; the UAT fixtures live in Documents/RKKP UAT Fixtures/.
  Checked By: Main agent

[2026-08-27 16:43] Phase 26 Compliance Check (pre-implementation)
  Changed: (planned, none written yet) src/commands/ingest.ts (per-PDF loop restructure + amendment path + audit/metrics), src/state/synthesis-state.ts (baselineKeys record extension + patch-amended mode), src/materializer.ts (evidence-key/delta export + merge-absorption surfacing), NEW src/agents/amendment.ts, NEW src/llm/patch.ts (patch schema + deterministic applier), NEW prompts/amendment.prompt.txt, src/validation/preservation-check.ts (merged-page check), src/state/metrics.ts (patched/fallback counters), tests/phase-26.test.ts, enumerated existing-test seam updates (phase-16/23 style, to be recorded as deviations), scripts/launcher-entry.ts (asset VERSION bump for the new prompt)
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md, 07_validation_and_quality.md, 01_PRODUCT_VISION_AND_ARCHITECTURE.md
  Sections Checked: 04 §1 (per-PDF sequential ingestion amendment, ratified 2026-08-26 — default run shape, DOX/workspace/cross-wiki/updater once after the loop), 04 §3.2 (loop scope: Steps 3-9 per PDF, checkpoint at each PDF's completion; Step 6 curation per PDF; Step 9 amendment synthesis — patch output, semantic diff by LLM, deterministic validate/apply/preserve, reask ≤3 then full-synthesis fallback, patch-amended skip-eligible, kind-change/shape-change full synthesis, contradiction blocks never delete), 04 §4 (amendment mode of the writer family, unioned preservation law), 04 §6 (patch quality-failure class: unparseable / bad anchor / merged-page preservation failure), 07 §3 (patched pages: merged-page preservation over old ∪ new verbatim, never half-patched), 01 §4.1 (curation per-PDF cadence, identity settled before the PDF's synthesis/amendment)
  Result: COMPLIANT — the 2026-08-26 vision amendments are law in the vision docs and the phase doc implements them without contradiction; the op vocabulary (add-evidence/add-section/add-member/edit-prose/flag-contradiction; no delete, no rewrite), the glm-5.3-flash-only live budget ($15 cap, gate 26.11 the one live gate), and the batch-pipeline removal all trace to the ratified amendments and the 2026-08-26/27 user decisions recorded in the root AGENTS.md preferences. No vision edits required.
  Checked By: orchestrator (main agent, ZCode)

[2026-08-27 ~20:45] Phase 26 Closeout — implementation, gates, live gate, UAT 26.1
  Changed: src/commands/ingest.ts (per-PDF loop: extract → materialize+per-PDF curation → synthesize-or-AMEND per PDF; DOX/workspace/cross-wiki/updater once after the loop; amendment eligibility incl. the curation-merge/cluster survivor veto; runAmendmentEpisode — writer → parsePatch → validatePatch → applyPatch → enforceMergedPage → the kind's preservation check over the full pageData (old ∪ new); reask ≤3 with exact errors; exhaustion/transport → full-synthesis fallback, never half-patched; patch-amended records with baselineKeys; amendment-log episodes; patchedPages/patchFallbacks metrics), src/state/synthesis-state.ts (baselineKeys + pageKind record fields; patch-amended skip-eligible), src/materializer.ts (per-item evidence-key builders + evidenceKeysFor + newEvidenceFor delta), NEW src/agents/amendment.ts (Amendment Writer, callType 'synthesis-amend' through the synthesis slot, temperature 0), NEW src/llm/patch.ts (closed op vocabulary schema + deterministic validator + order-stable applier), NEW src/state/amendment-log.ts (.state/amendment-log.jsonl), NEW prompts/amendment.prompt.txt, src/llm/client.ts ('synthesis-amend' in SYNTHESIS_CALL_TYPES), src/state/metrics.ts (patchedPages/patchFallbacks), scripts/launcher-entry.ts (VERSION 1.0.26→1.0.27 — the new prompt + amendment bundle are extracted assets), tests/phase-26.test.ts (gates 26.1–26.11 incl. the doc-gates 26.1 loop-order and 26.9 abort/resume that the original test file lacked), enumerated adaptations to tests/phase-16.test.ts (16.5/16.7/16.11), tests/phase-19.test.ts (19.4), tests/phase-23.test.ts (23.4/23.5), tests/phase-11.test.ts (11.5 README headings) — all recorded in .state/phase-26-status.json deviations, README.md (Phase-26-era lean progressive-disclosure rewrite, the root AGENTS.md structure), docs/
  Gates: all 11 green (tests/phase-26.test.ts 17 passed + 1 live-skipped key-less); the FULL key-less suite green — 38 files passed, 1 skipped (tests/e2e.test.ts), 577 tests: 561 passed, 16 skipped, 0 failed, EXIT 0; tsc --noEmit clean.
  Live gate 26.11: the REAL writeAmendment (no stub) against glm-5.3-flash (synthesis slot pinned { provider: 'zhipu', model: 'glm-5.3-flash' }), happy path on attempt 1 — schema-valid patch, merged-page preservation passed, llm-calls.json entry provider zhipu / model glm-5.3-flash / callType synthesis-amend. 1 call, 1427 in / 7505 out tokens (incl. reasoning), $0.003967. ZAI_API_KEY injected into process.env from dist/.paper-chase.json apiKeys.zhipu for the run only — never printed, never written to a file.
  UAT 26.1 (live two-PDF sequential build, glm-5.3-flash, workspace /tmp/paper-chase-uat26 with every routing slot pinned): see .state/phase-26-status.json for the cost line and the amendment-vs-creation output-token comparison.
  UAT 26.2 (kill-and-resume): proven deterministically by gate 26.9 (abort mid-PDF-3; resume re-processes only PDF 3; patched pages skip-eligible) — the live variant is budgeted but not re-run (the abort/resume mechanism is identical code to UAT 26.1's).
  UAT 26.3 (reading check): the amended page from UAT 26.1 rendered as one coherent article with both sources' evidence — human final check deferred to the Reporter.
  Budget: gate 26.11 $0.003967 + UAT 26.1 spend in the status file — far under the $15.00 cap (pause at $12.00); glm-5.3-flash ONLY for every live call (pinning verified in the UAT workspace config).
  Checked By: main agent (ZCode)

[2026-08-28 02:15] Workspace-registry bug fix — closeout (user-reported: a wiki created in a workspace other than the launch folder had its PDFs copied into the launch folder)
  Changed: src/tui/init-screen.tsx (onCreated payload { slug, workspace }), src/tui/app.tsx (workspace registry STATE seeded from the new workspaces/workspace props; flowWiki as WikiRef; onWorkspaceRegistered + test-only pickFolder threading), src/tui/add-pdfs-screen.tsx + src/tui/ingest-screen.tsx (workspaces?: string[] aggregation, WikiRef selection, per-selected-wiki workspace for copy/ingest/settings/language), src/tui/agents-review-screen.tsx (WikiRef list adaptation, single-workspace by design), src/tui/hooks/use-wiki-list.ts (useWikiList(workspaces: string[]): WikiRef[] — live scan of every registered workspace), src/tui/settings.ts (TuiSettings gains workspace + workspaces; parseSettings/normalizeWorkspaces), NEW src/tui/workspace-bootstrap.ts (loadWorkspaceRegistry + registerWorkspace with one-time settings migration), src/cli.ts (async TUI bootstrap action), NEW tests/tui/workspace-bootstrap.test.ts (8) + NEW tests/tui/app-workspace.test.tsx (4), tests/tui/add-pdfs-screen.test.tsx + tests/tui/ingest-screen.test.tsx + tests/phase-11.test.ts (WikiRef payload adaptations, assertion intents unchanged), .gitattributes (prompts/*.txt, templates/AGENTS.md, tests/snapshots/** pinned eol=lf), scripts/launcher-entry.ts (VERSION 1.0.27→1.0.28), AGENTS.md root (User Preferences 2026-08-28 + dist 1.0.28 entry), src/AGENTS.md (tui/cli bullets), tests/AGENTS.md (ownership + Verification)
  Vision Docs Checked: Project Vision/01_PRODUCT_VISION_AND_ARCHITECTURE.md, 03_DOX_concept_detailed.md, 04_orchestration_detailed.md
  Sections Checked: 04 §1/§3.2 (workspace as the parent of wikis/, per-wiki targets unchanged — the TUI change only routes existing commands to the correct workspace; no pipeline-order change), 03 §3.1 (folder structure unchanged), 01 §4 (TUI workflow: all core workflows from the TUI, the 2026-07-17 preference — multi-folder wikis now visible in one selector)
  Result: COMPLIANT — no vision contradiction; the fix is TUI-layer workspace bookkeeping only (no pipeline, schema, or output-format changes). The user ratified the registry location (launch folder's .paper-chase.json, not %APPDATA%) and the combined multi-workspace selector via AskUserQuestion.
  Verification: full key-less suite 577 passed + 16 skipped across 40 files, zero failures (the three CRLF-sensitivity failures — gates 25.2/26.2 + one prompt check — were a core.autocrlf=true checkout artifact of the 8f269d8 merge, fixed by re-normalizing the working copies to the committed LF and the scoped .gitattributes); npx tsc --noEmit clean (via dist/runtime-node.exe — npm not on PATH); .env stashed/restored byte-identical for the run.
  Checked By: main agent (ZCode)

[2026-08-30 ~22:30] Phase 16 v1.0.6 network/timeout stall ladder — closeout (user-ratified directive, born from the 2026-08-30 21:14 rkkp-dhhd abort: GLM-5.3-Flash `indikator-5` mega-composite exceeded the 600s headers timeout 3× and killed the run)
  Origin: user directive 2026-08-30 — keep trying on network/timeout errors, exact wait ladder 10/20/30/60/90 min, 6 attempts; model switching explicitly forbidden; honor Retry-After clamp already ratified; ratification via the user directive itself plus vision 04 §6 and 07 §5 amendments.
  Changed: src/llm/client.ts (NEW exported networkStallDelayMs(retry) with floor [10,20,30,60,90] min; LARGE_CALL_HEADERS_TIMEOUT_MS 600_000 → 900_000; retry loop treats opted-in thrown transport errors as networkStalled with TRANSIENT_MAX_ATTEMPTS=6 and the network floor; appendTransportStallLog writes statusCode:0 with the thrown message; StallWaitInfo.statusCode documented as 0 for network/timeout; live label "Connection problem (network/timeout)" surfaced via setStallWaitReporter), src/commands/ingest.ts (stall-reporter reason mapping for statusCode 0), Project Vision/04_orchestration_detailed.md §6 and Project Vision/07_validation_and_quality.md §5 (transient-transport retry clause amended to state both ladders: 429/5xx → 6 attempts, 1/5/15/45/90-min floor; network/timeout → 6 attempts, 10/20/30/60/90-min floor; Retry-After clamped to 90 min; maxRetries:0 frozen; fixes pre-existing drift where the 429/5xx ladder had never been written back into vision), tests/phase-16.test.ts (re-cast 16.8 headers-timeout pin to 900_000 and network-leg waits [600000,1200000]; 16.16 network/timeout 6-attempt ladder + maxRetries:0 immediate throw; 16.19 timeout leg 6 attempts with waits [600000,1200000,1800000,3600000,5400000]; NEW gate 16.22: networkStallDelayMs sequence, persistent HTTP/2 headers timeout → 6 attempts, transport-stalls.jsonl statusCode:0 records, reporter StallWaitInfo, ingest progress line), tests/tui/ingest-screen.test.tsx (NEW network-stall progress-line test, whitespace-normalized for 80-column wrap), tests/tui/app-workspace.test.tsx (line 148 pre-existing TS2769 generic-promise fix), scripts/launcher-entry.ts (VERSION 1.0.28→1.0.29), AGENTS.md root (2026-08-30 User Preference entry + dist 1.0.28→1.0.29 entry), src/AGENTS.md (llm/client.ts Phase 16 v1.0.6 bullet + four-class retry policy class-2 sentence updated), tests/AGENTS.md (ownership/Verification entry).
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md §6, Project Vision/07_validation_and_quality.md §5
  Sections Checked: the four-class retry policy — deterministic 4xx never retried; transient transport (429/5xx/network/timeout) retried with the user-ratified stall ladders; content-defect and quality reask unchanged; per-page transport fallback and outage detector unchanged.
  Result: COMPLIANT — the 2026-08-30 directive amends the transient-transport class of the four-class retry policy; the vision text now matches the implementation; model routing is untouched as required.
  Verification: full key-less suite 580 passed + 16 skipped across 40 files (39 passed + 1 skipped file — tests/e2e.test.ts), zero failures, zero unenumerated regressions; npx tsc --noEmit clean (via dist/runtime-node.exe — npm not on this shell's PATH); .env stashed to .env.keyless-tmp and restored byte-identical (171 bytes) for the key-less run.
  Notes: the JSON corrector (maxRetries:1) rides the same network stall ladder on network errors; the Settings test probe keeps its frozen 30s no-retry behavior. The network/timeout stall is live-labeled and audited with statusCode 0; exhausted stalls still abort loudly with the actual attempt count.
  Checked By: main agent (ZCode)

[2026-09-02 ~20:00] Phase 27 Pre-Check — Per-PDF Worker-Process Isolation
  Changed (planned): NEW src/commands/worker-protocol.ts, NEW src/tui/ingest-conductor.ts, NEW src/tui/worker-spawn.ts, NEW src/state/crash-log.ts, src/commands/ingest.ts (ingestOnePdf split + onlyPdfs/finalizeOnly selectors), src/cli.ts (ingest-worker subcommand), src/tui/ingest-screen.tsx (conductor swap + crash panel), tests/phase-27.test.ts, tests/tui/ingest-screen.test.tsx additions
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md §1/§3.2/§6, 01_PRODUCT_VISION_AND_ARCHITECTURE.md §4
  Sections Checked: 04 §1 (per-PDF loop + the new Worker-process isolation amendment — the phase's founding law), §3.2 (loop scope, checkpointing — retry resumes from per-PDF/per-page state), §6 (per-stage outage detector keeps its scope inside each worker; the conductor's auto-retry cap is the run-level circuit breaker), 01 §4 (TUI workflow — byte-identical screens in normal operation, crash panel is the only new UI)
  Result: COMPLIANT — founded on the user-ratified 2026-09-02 vision amendment (04 §1). No contradictions: pool cap 4 stays inside each worker (04 §1 concurrency law), workers strictly one at a time, state-file writes stay serialized (no new locking), retry semantics are Phase 16 checkpoint law, defer is user-initiated only and never automatic (the "data is never skipped" posture is preserved — deferral deletes nothing and the PDF is re-attempted next run), auto-retry cap 3 / 30s backoff / stop-and-surface as ratified. The 2026-09-02 post-mortem evidence (silent clean-exit death, zero crash telemetry, 135.8h continuous uptime) is recorded in the root AGENTS.md preference.
  Budget: $0.00 — fully deterministic phase (stub seams + PAPER_CHASE_WORKER_FAULT fault injection); the key-less suite profile is unchanged.
  Checked By: main agent (ZCode)

[2026-09-02 ~21:05] Phase 27 Closeout — Per-PDF Worker-Process Isolation (implemented; $0 LLM spend)
  Changed: src/commands/ingest.ts (engine split: onlyPdfs selector — a per-PDF worker run executes ONLY its selected PDFs' loop, the deferred tail is the finalize worker's alone; finalizeOnly mode — loop + fallback skipped, tail only; IngestResult.deferred additive field; formatIngestSummary deferred segment), NEW src/commands/worker-protocol.ts (JSONL event protocol — progress/result/fatal, noise-tolerant chunk-safe reader with flush; PAPER_CHASE_WORKER_FAULT fault-injection hooks), src/cli.ts (headless ingest-worker subcommand with console-to-stderr redirection so stdout carries only events), NEW src/tui/ingest-conductor.ts (runIngestConductor — spawn/relay/merge, user-ratified auto-retry cap 3×30 s then crash panel, never auto-defer, AbortSignal kill-forwarding; mergeIngestResults; discoverWorkspacePdfs), NEW src/tui/worker-spawn.ts (resolveWorkerCommand — dev tsx / staged bundle / pkg re-exec; PAPER_CHASE_WORKER_CMD override; loud missing-entry error), NEW src/state/crash-log.ts (.state/crash-log.jsonl via the serialized write queue; bounded 25-line stderr tails), src/tui/ingest-screen.tsx (production runs through the conductor; crash-recovery panel R/Skip-PDF/Abort; conductorFn test seam; SIGINT→AbortController), scripts/launcher-entry.ts (VERSION 1.0.29→1.0.30), NEW tests/phase-27.test.ts (12 tests / gates 27.1–27.11), tests/tui/ingest-screen.test.tsx (+2: healthy-run pixel-identity, crash-panel decide), tests/tui/phase-01-screens.test.tsx (adaptation: real ingest via the ingestFn seam), Project Vision/04 §1 (Worker-process isolation amendment, user-ratified 2026-09-02), AGENTS.md root (preference 2026-09-02 + dist 1.0.30 entry + Child DOX Index), src/AGENTS.md, tests/AGENTS.md, Implementation Plan/PHASE_27_per_pdf_worker_isolation.md + IMPLEMENTATION_PLAN_MASTER_INDEX.md
  Gates: all 11 green; FULL key-less suite green — 594 passed + 16 skipped across 41 files (40 passed + 1 skipped — tests/e2e.test.ts), zero failures, zero unhandled errors; npx tsc --noEmit clean (via dist/runtime-node.exe); .env stashed/restored byte-identical (171 bytes).
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md §1/§3.2/§6, 01_PRODUCT_VISION_AND_ARCHITECTURE.md §4
  Sections Checked: 04 §1 (the Worker-process isolation amendment — founding law; pool cap 4 inside each worker, workers strictly one at a time, state writes serialized; retry resumes from checkpoints; defer user-initiated only, never automatic; auto-retry cap then stop-and-surface; outage detector keeps its scope inside each worker), §3.2 (loop scope + checkpointing — the onlyPdfs cut is the amendment's own 'tail runs in its own worker' sentence made mechanical), §6 (per-stage outage detector unchanged inside workers), 01 §4 (TUI workflow — healthy runs byte-identical; crash panel is the only new UI; all core flows remain TUI-doable per the 2026-07-17 preference)
  Result: COMPLIANT — the implementation matches the user-ratified amendment on every clause; gate 27.1 proves the conductor shape (PDF worker ×2 + finalize run) converges byte-for-byte to the single full run's durable tree, so no pipeline law drifted across the process boundary.
  Budget: $0.0000 — every gate deterministic (stub seams + scripted spawns); no gate requires any API key; live UATs are perceptual reads of stub-driven scratch wikis.
  Checked By: main agent (ZCode)

[2026-09-03 ~08:00] Phase 27 v1.0.1 Pre-Check — Worker-scope fencing + conductor observability (user-ratified 2026-09-03: AskUserQuestion round "All three fixes / Full TUI depth / Rebuild ASAP" + rollout directive "Roll this out according to the change flow of /canon"; born from the 2026-09-03 live rkkp evidence: the AKDB skip-worker printed four FALSE "PDF no longer in raw/" warnings because the Phase 8 orphan check reads the onlyPdfs-filtered file list, and the AFDK skip-worker burned 8h53m re-curating the whole wiki because the 2026-07-21 all-skip repair fallback re-runs in EVERY hash-skip worker under the conductor)
  Changed (planned): src/commands/ingest.ts (orphan-check + empty-raw gating on onlyPdfs===undefined; fallback re-gated to full runs OR finalizeOnly+idleFallback; additive idleFallback option + onStall tap), src/cli.ts (--idle-fallback flag, stall event emission), src/commands/worker-protocol.ts (stall event type), src/tui/ingest-conductor.ts (per-worker banners, onWorkerChange, --idle-fallback pass-through keyed on merged ingested.length===0, stall relay), src/llm/client.ts (StallWaitInfo.label), src/tui/ingest-screen.tsx (worker-position status row, live stall countdown row), Project Vision/04 §1 (2026-09-03 observability rider), AGENTS.md root + src/AGENTS.md + tests/AGENTS.md, Implementation Plan/PHASE_27_per_pdf_worker_isolation.md (v1.0.1 section), scripts/launcher-entry.ts (VERSION 1.0.30→1.0.31), tests/phase-27.test.ts + tests/tui/ingest-screen.test.tsx gates
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md §1/§3.2, Project Vision/01_PRODUCT_VISION_AND_ARCHITECTURE.md §4, Project Vision/07_validation_and_quality.md §5 (stall-line wording provenance)
  Sections Checked: 04 §1 Worker-process isolation amendment (byte-identical clause + "only new UI is the crash panel"), 04 §1 per-PDF sequential ingestion (fallback semantics), 01 §4 (TUI workflow law)
  Result: COMPLIANT WITH ONE USER-RATIFIED AMENDMENT —
    (a) Orphan-check fencing: COMPLIANT, restores Phase 8 law (the check's object is the raw/ directory, not the worker's scope); the finalize worker carries the unfiltered list, so the check runs exactly once per run with correct data.
    (b) Fallback fencing + idleFallback: COMPLIANT, restores the 2026-07-21 all-skip repair law's batch semantics ("one more ingest repairs") under the conductor — exactly ONE repair pass per all-skip run, in the finalize worker, instead of one per hash-skip worker. The conductor is the only party that knows whether any PDF landed (merged IngestResult.ingested).
    (c) Observability (banners, worker-position row, stall label + live countdown): CONTRADICTS the 2026-09-02 amendment's "user-facing screens stay byte-identical in normal operation: ... the only new UI is the crash-recovery panel" — resolved by the user's explicit 2026-09-03 ratification (Full TUI depth); vision 04 §1 gains the 2026-09-03 rider recording the three sanctioned additions. The stall-line wording extension (label prefix) amends the 2026-08-20/22/30 preference entries' pinned format and is recorded in the root AGENTS.md preference log.
  Budget: $0.00 — fully deterministic (stub seams + scripted spawns); the key-less suite profile is unchanged.
  Checked By: main agent (ZCode)

[2026-09-03 ~10:00] Phase 27 v1.0.1 Closeout — Worker-scope fencing + conductor observability (implemented; $0 LLM spend)
  Changed: src/commands/ingest.ts (orphan-check + empty-raw warnings gated on options.onlyPdfs === undefined; all-skip repair fallback re-gated to full runs OR finalizeOnly && idleFallback; additive idleFallback option / IngestOptions.idleFallback + onStall tap; stall reporter forwards call label), src/cli.ts (--idle-fallback flag for ingest-worker --finalize; onStall emits {type:'stall'} worker event), src/commands/worker-protocol.ts ({type:'stall', info: StallWaitInfo} added to WorkerEvent union, parsing, and serialization), src/tui/ingest-conductor.ts (per-worker banner lines emitted before every spawn attempt; onWorkerChange option; idleFallback = merged ingested.length === 0 and passed only to finalize worker; stall events relayed to onStall), src/llm/client.ts (StallWaitInfo.label populated from options.context ?? options.callType), src/tui/ingest-screen.tsx (workerPos state, stallWait state with 1 s heartbeat; dim worker-position row "Worker i/(total+1) · <pdf> · elapsed …"; yellow live stall countdown row with zero-clamp to "retry in flight…"), Project Vision/04_orchestration_detailed.md §1 (2026-09-03 rider codifying the three sanctioned additions: orphan-check fencing, all-skip fallback fencing, conductor observability), AGENTS.md root (2026-09-03 preference entry + dist 1.0.31 log), src/AGENTS.md (phase-27 v1.0.1 notes), tests/AGENTS.md (gate inventory update), Implementation Plan/PHASE_27_per_pdf_worker_isolation.md (v1.0.1 section + approval checklist), Implementation Plan/IMPLEMENTATION_PLAN_MASTER_INDEX.md (Phase 27 row v1.0.1), .state/phase-27-status.json (status implemented, version 1.0.1, all gates passed), scripts/launcher-entry.ts (VERSION 1.0.30→1.0.31), tests/phase-27.test.ts (gates 27.12–27.19), tests/tui/ingest-screen.test.tsx (worker-position + live-stall rows), tests/phase-16.test.ts (gate 16.17 regex + gate 16.22 label assertion updated)
  Gates: 27.1–27.19 all green; FULL key-less suite green — 602 passed + 16 skipped across 42 files (41 passed + 1 skipped — tests/e2e.test.ts), zero failures, zero unhandled errors; npx tsc --noEmit clean (via dist/runtime-node.exe); .env stashed to .env.keyless-tmp and restored byte-identical (171 bytes); independent Verifier sub-agent cold-check PASS.
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md §1 (2026-09-03 rider), Project Vision/01_PRODUCT_VISION_AND_ARCHITECTURE.md §4, Project Vision/07_validation_and_quality.md §5
  Sections Checked: 04 §1 Worker-process isolation amendment (orphan-check fencing: check runs once per run at finalize; fallback fencing: one repair pass per all-skip run; observability: banners/status/stall rows sanctioned by the 2026-09-03 rider), §3.2 per-PDF loop + checkpointing (idleFallback lets the finalize worker know the merged ingested count), 07 §5 (stall-line label prefix is a live-label refinement, not a wording contradiction)
  Result: COMPLIANT — the three live-rkkp defects are resolved without contradicting the amended vision; the orphan check and all-skip fallback are fenced to run-level scope; the conductor now exposes meaningful progress while preserving the crash-panel contract.
...
[2026-09-07 09:49] Phase 28 (backlog B25) Compliance Pre-check — fine-grained crash resume: per-chunk extraction checkpoints + per-PDF stage markers + in-flight synthesis journal + crash telemetry (docs pass; implementation pending)
  Changed (planned): NEW src/state/extraction-checkpoints.ts (readValidExtraction — parse + _provenance envelope check + deterministic validateExtractorResult re-validation, no LLM), src/commands/extract-chunk.ts (_provenance envelope {sha256, pages, sourceFile, extractedAt} written from the document-page frontmatter; sources[0].sha256 read instead of ignored), src/commands/ingest.ts (chunk-loop skip guard BEFORE the extractChunkFn ?? extractDocumentChunk seam, applied regardless of seam; dim resume line on a PDF's first skip; result.extractions + metrics counted from stored JSON for skipped chunks; changed-PDF stale cleanup deletes an old chunk JSON only when its envelope is absent or hash-mismatched; NEW src/state/pdf-progress.ts consumption — stages {extraction, materialize, curation} + extractedSetHash fingerprint, entry removed at the ingestion.json checkpoint; runSynthesisStages journals {stage cursor, done, queue} per page, written after the per-page record), NEW src/state/pdf-progress.ts (.state/pdf-progress.json via enqueueSerializedWrite), src/cli.ts (worker catch also console.error's the error → worker stderr), src/tui/ingest-conductor.ts (fatal events captured instead of dropped → crash record fatalError/fatalStack), src/state/crash-log.ts (additive fatalError/fatalStack; tailLines filters 'LLM Call |' cost lines before the last-25 cut), src/tui/ingest-screen.tsx (crash panel renders fatalError preferentially), tests/phase-28.test.ts (gates 28.1–28.9), tests/phase-16.test.ts (aborted-PDF expectation amended to per-chunk resumption + .state/phase-16-status.json deviation note), stub helpers (makeExtractChunkFnStub / stubExtractChunkFn write the envelope for fidelity), scripts/launcher-entry.ts (VERSION 1.0.31→1.0.32 at implementation closeout)
  Docs changed (this pass): Project Vision/04_orchestration_detailed.md §1 (2026-09-07 fine-grained-crash-resume rider) + Step 11 (checkpointing extension), Implementation Plan/PHASE_28_fine_grained_crash_resume.md (NEW — gates 28.1–28.9), Implementation Plan/BACKLOG.md (B25 refined + removed — scheduled), Implementation Plan/IMPLEMENTATION_PLAN_MASTER_INDEX.md (Phase 28 row), Implementation Plan/AGENTS.md (Phase 28 scheduled note), .state/phase-28-status.json (NEW — pending) + .state/phase-27-status.json (unchanged)
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md §1 (worker-isolation amendment + 2026-09-03 rider), §3.2 Step 5 (extraction), Step 9 (synthesis resume 2026-07-25 + amendment synthesis 2026-08-26), Step 11 (checkpointing 2026-07-25), Project Vision/01_PRODUCT_VISION_AND_ARCHITECTURE.md §4 (TUI workflow), Project Vision/07_validation_and_quality.md §2.3/§5 (validator reuse provenance; no transport change)
  Sections Checked: 04 §1 retry clause ("resuming from the per-PDF/per-page checkpoints, law unchanged") — extended one granularity down by the user-ratified 2026-09-07 refinement round; 04 Step 11 ("an aborted run never re-extracts finished PDFs") — the same guarantee carried INSIDE an in-flight PDF (chunks/stages/pages), extension not contradiction; 04 Step 9 per-page records — remain the authoritative durable skip law, the journal is an in-run cursor written consistently after each record; 01 §4 + 04 §1 byte-identity clause — the resume line is recovery-path only (2026-09-03 observability rider precedent), normal runs unchanged; 07 §2.3 — validateExtractorResult reused deterministically (the reask loop's own validator), zero new LLM surface
  Result: COMPLIANT (user-ratified extension) —
    (a) Per-chunk checkpoints: COMPLIANT, extends the Step 11 checkpointing law in its own direction ("data never skipped" — a valid on-disk extraction JSON is data, not skipped data; the changed-PDF re-ingest path keeps deleting stale JSONs, now never deleting a fresh-hash checkpoint).
    (b) Stage markers + synthesis journal: COMPLIANT, additive in-run state that makes the EXISTING per-page resume effective (today a retry's chunk re-extraction churn shifts page fingerprints and silently defeats it); the per-PDF ingestion record and per-page records stay authoritative; the journal is deleted at the PDF's checkpoint.
    (c) Resume line: recovery-path-only progress line, sanctioned by the 2026-09-03 observability rider pattern; normal-operation byte-identity intact.
    (d) Crash telemetry: the crash-log shape is Phase 27's own artifact (no vision clause governs its fields) — additive fatalError/fatalStack + cost-line filtering make the 2026-09-02 "captures the worker's stderr" intent actually carry the exception.
    Superseded test-law note: the phase-16 gate comment "the aborted PDF (pdf-b) is re-processed whole (per-PDF atomicity — its chunks re-extract)" is superseded by the ratified Step 11 extension; the test expectation will be amended at implementation with a deviation note in .state/phase-16-status.json (tests/AGENTS.md contract).
  Budget: $0.00 — fully deterministic (stub seams + scripted spawns + fault injection); the key-less suite profile is unchanged.
  Refinement gate: PASSED 2026-09-07 (AskUserQuestion round — envelope store / chunks+stage markers / full telemetry / one dim resume line / coverage through synthesis-amendment / explicit in-flight journal over verify-only; free-text scope expansion "killed in later phases → resume where it ended, e.g. during synthesis" drove outcomes 2+3). Outcomes recorded in the PHASE_28 doc canon basis; B25 removed from the backlog per the scheduling discipline.
  Checked By: main agent (ZCode, canon doc pass)

[2026-09-07 ~14:10] Phase 28 Closeout — Fine-grained crash resume (implemented; $0 LLM spend)
  Changed: NEW src/state/extraction-checkpoints.ts (readValidExtraction — parse + _provenance envelope check + deterministic validateExtractorResult re-validation; readExtractionProvenanceSha256 for the envelope-aware stale cleanup), NEW src/state/pdf-progress.ts (.state/pdf-progress.json stage markers via enqueueSerializedWrite; synthesis journal ops — begin/append/complete; computeExtractedSetHash; materialize-restore cache save/load; entry + cache-dir removal at the PDF's checkpoint), src/commands/extract-chunk.ts (_provenance envelope written first from the document-page frontmatter incl. sources[0].sha256), src/commands/ingest.ts (chunk-loop guard before the seam regardless of stub; pre-scan resume line 'Resuming <file> — N/M chunks already extracted, skipping...'; counts/metrics from stored JSON for skipped chunks; envelope-aware changed-PDF cleanup preserves fresh-hash checkpoints; extraction/materialize/curation stage markers with extractedSetHash fingerprint; materialize-skip + cache restore (restoreMaterializeResult — disk-converged hashes, writtenPages deliberately empty); synthesis journaling in runSynthesisStages (per-PDF only, journal AFTER the per-page record; completeSynthesisJournal at stage exhaustion); removePdfProgressEntry at the ingestion checkpoint), src/cli.ts (worker catch also console.error's the error — stderr mirror), src/tui/ingest-conductor.ts (fatal events captured — not dropped — into WorkerOutcome.fatal → crash record fatalError/fatalStack + CrashPanelState), src/state/crash-log.ts (additive fatalError/fatalStack; tailLines filters 'LLM Call |' cost lines before the last-25 cut), src/tui/ingest-screen.tsx (crash panel renders 'Fatal error: …' preferentially), scripts/launcher-entry.ts (VERSION 1.0.31→1.0.32), NEW tests/phase-28.test.ts (10 tests, gates 28.1–28.8), tests/tui/ingest-screen.test.tsx (+1 fatalError panel test), tests/phase-16.test.ts (gate 16.11 amended: the aborted PDF's completed chunks now RESUME — leg2ExtractionCalls [] — superseded per-PDF-atomicity comment updated; deviation recorded in .state/phase-16-status.json), tests/phase-08/16/27 stub helpers write the _provenance envelope (phase-27 gained the missing gray-matter import), tests/AGENTS.md + src/AGENTS.md + root AGENTS.md (2026-09-07 preference entry + dist 1.0.32 chain) + Implementation Plan/AGENTS.md + IMPLEMENTATION_PLAN_MASTER_INDEX.md + PHASE_28 doc header/checklist, .state/phase-28-status.json (implemented-pending-verification), .state/phase-16-status.json (deviation note)
  Gates: 28.1–28.9 all green; FULL key-less suite green — 613 passed + 16 skipped across 42 files (41 passed + 1 skipped — tests/e2e.test.ts), zero failures; tsc --noEmit clean (via dist/runtime-node.exe); .env stashed to .env.keyless-tmp and restored byte-identical (171 bytes). Independent Verifier cold-check PENDING (the Implementer sub-agent hit the account 5-hour usage limit mid-run after completing all source code; the orchestrating agent finished tests + closeout — role-merge deviation recorded in .state/phase-28-status.json).
  Vision Docs Checked: Project Vision/04_orchestration_detailed.md §1 (2026-09-07 Fine-grained crash resume rider), §3.2 Step 9 (per-page records remain the law), Step 11 (fine-grained resume extension), Project Vision/01_PRODUCT_VISION_AND_ARCHITECTURE.md §4 (byte-identity), Project Vision/07_validation_and_quality.md §2.3/§5
  Sections Checked: 04 §1 rider clauses (1) per-chunk checkpoints — implemented exactly (parse + envelope + deterministic schema re-validation; changed-PDF cleanup never deletes a fresh-hash checkpoint), (2) stage markers keyed to extractedSetHash — implemented with the materialize-restore cache companion (a materialize skip must still feed synthesis its aggregates; cache written before the marker so a crash between degrades to a conservative re-materialize), (3) in-flight synthesis journal — observer-only, entries written AFTER the per-page records, closed at stage exhaustion, entry removed at the PDF's checkpoint; Step 11 extension (chunks/stages/pages never repeated after a crash) — proven by gates 28.4/28.6/28.7; Step 9 per-page records — untouched and still authoritative (gate 28.7 proves a fingerprint-mismatched page re-synthesizes despite a journal done-entry); 01 §4 byte-identity — the resume line is recovery-path only (gate 28.3: no resume lines, no scaffolding, on a healthy run; pre-existing byte-asserting suites green); crash observability clause — fatalError/fatalStack additive (absent without a fatal event — gate 27.6's exact-equal still green), cost-line filter before the last-25 cut, worker stderr mirror (gate 28.8c proves the real subprocess path end-to-end)
  Result: COMPLIANT — the implementation matches the user-ratified 2026-09-07 rider and Step 11 extension with no contradictions; the two gating-discovered defects were fixed IN FAVOR of the law (the lingering .state/pdf-progress/ cache dir violated the byte-identity clause; the draft resume-line wording misnamed the resume point — replaced with the refinement round's ratified wording).
  Budget: $0.0000 — every gate deterministic (stub seams + scripted spawns + one real worker subprocess against a corrupted-state wiki that throws before any LLM call); no API key required.
  Build Status: DONE — VERSION 1.0.32 rebuilt via the runtime node (npm not on this shell's PATH), exe smoke-tested (1.0.32 runtime extracted, ingest-worker subcommand present); UAT ACCEPTED by the user 2026-09-07; commits feat 63fbc1e + build a3e8552 landed; docs commit to follow.
  Checked By: main agent (ZCode) — Verifier cold-check to follow as a separate pass

[2026-09-07 ~15:45] Phase 28 Closeout Addendum — independent Verifier cold-check PASS
  Report: .state/phase-28-verification.md (fresh Verifier sub-agent, cold — no implementation rationale shared).
  Machine checks (independent re-run): FULL key-less suite green — 613 passed + 16 skipped across 42 files (41 passed + 1 skipped — tests/e2e.test.ts), zero failures; focused re-run of phase-28 + phase-16 + ingest-screen 47/47 green; tsc --noEmit clean; .env stashed/restored byte-identical (171 bytes verified).
  Per-gate verdicts: 28.1-28.9 ALL PASS (incl. the non-vacuous anchors: gate 28.6 asserts materialize spy 0 calls + zero curation calls, not just a line; gate 28.7 proves the fingerprint-mismatch override re-synthesizes despite the journal done-entry; gate 28.8c runs a REAL worker subprocess and sees the fatal event on stdout + the same error on stderr + exit 1; gate 28.9's doc items — vision rider + Step 11 extension, root preference + dist chain, phase-16 deviation note — verified present and accurate).
  Findings F1-F6 (all non-blocking): F1 — phase doc carried the draft resume-line wording; corrected to the ratified pre-scan wording same-day (the implemented line matches the root AGENTS.md entry). F2 — 28.1's zero-relationships fixture leaves the metrics-accumulation lines un-pinned with non-zero values (correct by inspection). F3 — the conductor's CrashPanelState fatal-field spread is never exercised end-to-end (28.8b auto-retries; the screen test injects state directly; the crash-record spread IS tested). F4 — checkpointed chunks re-emit their 'Chunk N/M' progress from 1 (the UAT draft's 'counter continues' phrasing vs the implemented behavior; user accepted). F5/F6 — byte-identity proof is compositional; the screen test asserts presence (not ordering) of the fatalError row. F2/F3 noted as future test-hardening candidates.
  Phase 28 is now FULLY CLOSED (all checklist items ticked).
  Checked By: independent Verifier sub-agent + main agent (ZCode, closeout)
