...
* `synthesis-document.prompt.txt` — Phase 5: the strict document Synthesis Writer prompt. System instructions for a two-layer document summary with readable synthesis at the top and the verbatim extracted text, tables, figure descriptions, and `## Sources` below + runtime placeholders `{title}`, `{sourceFile}`, `{pageRange}`, `{extractedText}`, `{entities}`, `{claims}` substituted by `src/agents/synthesis.ts`; the wiki constitution is appended by the agent
* `synthesis-document-permissive.prompt.txt` — Phase 5: the permissive document Synthesis Writer fallback prompt. The model may summarize in Layer 1 but must preserve the exact extracted text, tables, and figure descriptions verbatim in the preserved-detail sections
